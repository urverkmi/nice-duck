/*
 * LVGL drawing helpers + canvas rotation. Adapted from the stock zmk nice_view
 * widget (MIT, The ZMK Contributors) for LVGL 9.
 *
...
/*
 * The panel is mounted rotated, so we draw everything in a natural orientation
 * into a square canvas and rotate it 270 degrees before it is shown.
 */
...
void draw_battery(lv_obj_t *canvas, lv_coord_t x, lv_coord_t y, uint8_t level, bool charging) {
    ...
    canvas_draw_rect(canvas, x + 0, y + 0, 29, 12, &rect_white_dsc);
    canvas_draw_rect(canvas, x + 1, y + 1, 27, 10, &rect_black_dsc);
    canvas_draw_rect(canvas, x + 2, y + 2, (level + 2) / 4, 8, &rect_white_dsc);
    canvas_draw_rect(canvas, x + 30, y + 3, 3, 6, &rect_white_dsc);
    canvas_draw_rect(canvas, x + 31, y + 4, 1, 4, &rect_black_dsc);

    if (charging) {
        lv_draw_image_dsc_t img_dsc;
        lv_draw_image_dsc_init(&img_dsc);
        canvas_draw_img(canvas, x + 9, y - 3, &bolt, &img_dsc);
    }
}
